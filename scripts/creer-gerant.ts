 ...
 *
 * Le mot de passe est demandé à la saisie, sans écho. Il peut aussi venir de
...
async function main() {
  ...
  const telSaisi = argument("tel") ?? process.env.TELEPHONE;

  if (!telSaisi) {
    console.error("Numéro manquant.  Exemple :");
    console.error('  npm run creer-gerant -- --tel "+224620000000" --nom "Mamadou Bah"');
    process.exit(1);
  }
